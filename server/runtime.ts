...
import { acquireServiceLease, createSharedServiceGuard } from './db/lease.js';
...
export async function createRuntime(
  ...
  let lease: ReturnType<typeof acquireServiceLease> | undefined;
  let heartbeat: ReturnType<typeof setInterval> | undefined;
  try {
    lease = (options.exclusive ?? options.recover !== false) ? acquireServiceLease(db) : undefined;
    ...
    const beginShutdown = () => {
      ...
      // A stale process must not stop work authorized by a replacement service.
      const active = lease?.isOwner()
        ? (db.prepare("SELECT id FROM runs WHERE status IN ('queued','running')").all() as {
            id: string;
          }[])
        : [];
      // Revoke shared-process signing authority before draining asynchronous work.
      lease?.release();
