...
> Historical audit through September 16. The September 19 release corrected the A logo and implemented network-bound mainnet support. See the [September 20 release report](release-2026-09-20.md) and [integration matrix](integration-status.md) for current checks, deployment and unresolved live prerequisites. Earlier test counts, logo geometry and devnet-only statements below are dated history.
...
| Controlled tests                      | 80 passed across 9 files in the final full `npm test`                                                                                                                                                              |
...
| Browser journeys                      | All 6 passed together in the final local suite: public flows plus real authenticated private access, media behavior and deterministic recovery in isolated controlled servers                                                                                        |
| Public browser coverage               | Landing, keyboard CTA, operator gating, developer guide, two fixture purchases, separate probe, export, print, empty history, service failure, ambiguous settlement recovery, refresh reset, 390px, reduced motion and 200% CSS zoom              |
...
The deployment follow-up passed lint, TypeScript, both builds, all 80 controlled tests, six normal browser journeys and four hosted journeys. An independent reviewer approved the deployment boundary, routing, Content Security Policy, print fix and recovery semantics. A print regression test confirms newly revealed timeline entries are immediately visible without waiting for screen animations.
...
The design follow-up passed lint, frontend/backend TypeScript, all 80 controlled tests, normal and static builds, six normal browser journeys, and four local hosted journeys. The first brand test run found a Node ESM JSON import incompatibility; the native import attribute fixed it, and the full hosted suite then passed. No paid API or payment test was used.
...
On September 15, 2026, the repaired A14_RED candidate was explicitly promoted to production as `dpl_2PH5GUh1Cefaxc2WtEJbkeAS96Xb`. Four hosted browser journeys passed against `https://allowanceonsolana.vercel.app`. The public boundary now serves the boundary-and-dot favicon, both selected films and the regenerated brand ZIP. Nine checked asset hashes match the local rehearsal build; `connect-src 'none'`, same-origin `media-src 'self'`, and API exclusion remain intact. Evidence is in [production-deployment-2026-09-15.json](../evidence/production-deployment-2026-09-15.json). This remains a static public rehearsal; no payment or model runtime is present.

The final recovery-aware build from source commit `48432f6` was promoted afterward as `dpl_AHkuAUhTqPcLJfJXshWCG1LWfRLs` and aliased to [allowanceonsolana.vercel.app](https://allowanceonsolana.vercel.app). Four hosted journeys passed, and a public recovery check confirmed an interrupted ambiguous run, a `settled-but-result-unavailable` receipt, an amber recovery state and no external requests. The final deployment record is [production-deployment-2026-09-15-final.json](../evidence/production-deployment-2026-09-15-final.json). The public site remains a static rehearsal; no payment or model runtime is present.

On September 16, 2026, the product-description and rehearsal-copy update from source commit `47c9661` was promoted as `dpl_34cQeJh8x2GSUudFSeSCrMY9yeYe`. The public HTML now carries the short product description, all four hosted journeys passed, the live demo copy names the useful purchases, separate denial and recovery states, and the public recovery check still shows no external requests. Evidence is in [production-deployment-2026-09-16-final.json](../evidence/production-deployment-2026-09-16-final.json) and [production-2026-09-16-recovery.png](../evidence/production-2026-09-16-recovery.png). The public site remains a static rehearsal; no payment or model runtime is present.

The UI refresh from source commit `fc4f093` was promoted as `dpl_wkBdsvHWKVCNDg4xA6DFuAy3ULQE`. Public verification confirmed the cache-busted A mark favicon, no film pause controls, autoplaying same-origin films, readable receipt copy, animated flow diagram, guided working example, mainnet-preview labels, four hosted journeys and no external requests. Evidence is in [production-deployment-2026-09-16-ui-refresh.json](../evidence/production-deployment-2026-09-16-ui-refresh.json). This remains a static no-spend rehearsal; no mainnet transaction or paid model call was performed.
