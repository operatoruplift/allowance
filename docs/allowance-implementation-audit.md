...
- `npm test` — **79 tests passed across 9 files**.
...
Baseline audit commit before the repair: `9fbac6b` (`Stabilize MCP stdio integration test`). The final audit commit is the commit containing this report, titled `Audit and complete A14 red interface`; the repository HEAD records the exact hash. The reproducible build outputs are the hashes listed above; generated brand kit manifest version is `2026-09-15`.
