...
On September 15, 2026, the final recovery-aware A14_RED candidate from source commit `48432f6` was promoted as `dpl_AHkuAUhTqPcLJfJXshWCG1LWfRLs` and aliased to [allowanceonsolana.vercel.app](https://allowanceonsolana.vercel.app). Four hosted browser journeys passed against the public URL, plus a public recovery check that preserved an interrupted run and `settled-but-result-unavailable` receipt without external requests. The updated boundary-and-dot favicon, canonical symbol, Axiom and Constellation films, posters and regenerated brand kit are live. Nine checked asset hashes match the local static build; `connect-src 'none'` remains enforced and `/api/health` returns 404. The deployment is still a deterministic rehearsal with no backend, signer, MCP process, OpenAI key or payment endpoint. Evidence is in [production-deployment-2026-09-15-final.json](../evidence/production-deployment-2026-09-15-final.json), [production recovery screenshot](../evidence/production-2026-09-15-recovery.png) and the prior matched public screenshots under `evidence/production-2026-09-15-*`.
