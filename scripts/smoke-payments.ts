...
export async function runPaymentSmoke(network: PaymentNetwork) {
  if (!process.argv.includes(`--confirm-${network}-spend`)) {
    process.stderr.write(
      `Not run. This command can spend 0.030000 ${network === 'mainnet' ? 'real mainnet' : 'test devnet'} USDC and requires a dedicated funded key, separate recipient, trusted sponsor, and DEMO_WALLET. Opt in with: npm run smoke:${network} -- --confirm-${network}-spend\n`
    );
    process.exit(2);
  }
  const config = loadConfig();
  if (!config.liveEnabled || config.paymentNetwork !== network)
    throw new Error(
      `This script requires LIVE_PAYMENTS_ENABLED=true and PAYMENT_NETWORK=${network}. It cannot spend on another network.`
    );
  ...
  try {
    ...
    runtime.ledger.setReport(
      run.id,
      `Scripted ${network} integration check. Wallet facts and transaction explanation appear in the purchased results. This is not an autonomous LLM trace.`
    );
    await fs.mkdir('evidence', { recursive: true });
    const filename = `evidence/${network}-smoke-${new Date().toISOString().replaceAll(':', '-')}.json`;
    await fs.writeFile(
      filename,
      JSON.stringify(
        {
          recordedAt: new Date().toISOString(),
          execution: `scripted live ${network} smoke; no LLM`,
          demonstrated,
